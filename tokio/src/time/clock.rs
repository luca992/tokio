...
//!
//! By default, `web_time::Instant::now()` is used. However, when the
...
cfg_test_util! {
    ...
    struct Inner {
        ...
        unfrozen: Option<web_time::Instant>,
    ...
    pub fn resume() {
        with_clock(|maybe_clock| {
            ...
            inner.unfrozen = Some(web_time::Instant::now());
            Ok(())
        });
    }
    ...
    pub(crate) fn now() -> Instant {
        if !DID_PAUSE_CLOCK.load(Ordering::Acquire) {
            return Instant::from_std(web_time::Instant::now());
        }
